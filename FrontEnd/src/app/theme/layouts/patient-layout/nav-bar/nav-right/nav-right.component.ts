...
@Component({
  ...
  host: { 'data-layout': 'patient', 'data-nav': 'nav-right' }
})
export class NavRightComponent {
  ...
  /** Stub for notification UI (no backend service yet). */
  notificationService = {
    unreadCount: 0,
    formatDate: (d: string) => (d ? new Date(d).toLocaleDateString() : '')
  };
  notificationsLoading = false;
  notifications: { id: number; title: string; message: string; read: boolean; createdAt: string; type?: string }[] = [];
  markAsRead = (_: { id: number }) => {};
